...
func TestEndpointHeartbeatAlertsEscalateWithoutDuplication(t *testing.T) {
	...
	alerts := endpointAlerts(endpoint, settings, false, now)
	...
	if alerts = endpointAlerts(endpoint, settings, false, now); len(alerts) != 0 {
		t.Fatalf("disabled heartbeat alerts = %#v", alerts)
	}
	settings.Enabled = true
	if alerts = endpointAlerts(endpoint, settings, true, now); len(alerts) != 1 || alerts[0].Code != "heartbeat_suppressed_maintenance" {
