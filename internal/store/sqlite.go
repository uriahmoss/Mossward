...
func (s *SQLiteStore) migrate() error {
	...
	if current < 31 {
		if err := s.applyScannerWorkerDispatchMigration(); err != nil {
			return err
		}
	}
	if current < 32 {
		if err := s.applyScanPolicyExecutionMigration(); err != nil {
			return err
		}
	}
	if current < 33 {
		if err := s.applyScannerWorkerDeadLetterMigration(); err != nil {
			return err
		}
	}
	if current < 34 {
		if err := s.applyCheckCatalogMigration(); err != nil {
			return err
		}
	}
	if current < 35 {
		if err := s.applyIntrusiveCheckPolicyMigration(); err != nil {
			return err
		}
	}
	if current < 36 {
	...
	if current < 38 {
		if err := s.applyEndpointCollectorPolicyMigration(); err != nil {
			return err
		}
	}
	if current < 39 {
		if err := s.applyAgentUpdateCatalogMigration(); err != nil {
			return err
		}
	}
	if current < 40 {
		if err := s.applyAgentUpdateAssignmentMigration(); err != nil {
			return err
		}
	}
	if current < 41 {
		if err := s.applyAgentModuleMigration(); err != nil {
			return err
		}
	}
	if current < 42 {
		if err := s.applyEndpointOSInventoryMigration(); err != nil {
			return err
		}
	}
	if current < 43 {
		if err := s.applyEndpointSoftwareInventoryMigration(); err != nil {
			return err
		}
	}
	if current < 44 {
		if err := s.applyEndpointListeningInventoryMigration(); err != nil {
			return err
		}
	}
	if current < 45 {
		if err := s.applyEndpointPostureMigration(); err != nil {
			return err
		}
	}
	if current < 46 {
		if err := s.applyEndpointCVEMigration(); err != nil {
			return err
		}
	}
	if current < 47 {
		if err := s.applyEndpointNetworkMigration(); err != nil {
			return err
		}
	}
	if current < 48 {
		if err := s.applyEndpointNetworkProcessMigration(); err != nil {
			return err
		}
	}
	if current < 49 {
		if err := s.applyEndpointNetworkContextMigration(); err != nil {
			return err
		}
	}
	if current < 50 {
		if err := s.applyThreatIndicatorMigration(); err != nil {
			return err
		}
	}
	if current < 51 {
		if err := s.applyEndpointNetworkExclusionMigration(); err != nil {
			return err
		}
	}
	if current < 52 {
		if err := s.applyEndpointCoverageMigration(); err != nil {
			return err
		}
	}
	if current < 53 {
		if err := s.applyCoverageDiscoveryPolicyMigration(); err != nil {
			return err
		}
	}
	if current < 54 {
		if err := s.applyAssetAgentEligibilityMigration(); err != nil {
			return err
		}
	}
	if current < 55 {
		if err := s.applyEndpointHeartbeatSettingsMigration(); err != nil {
			return err
		}
	}
	if current < 56 {
		if err := s.applyEndpointIntegrityMigration(); err != nil {
			return err
		}
	}
	if current < 57 {
		if err := s.applyEndpointIntegritySequenceMigration(); err != nil {
			return err
		}
	}
	if current < 58 {
		if err := s.applyEndpointMaintenanceMigration(); err != nil {
			return err
		}
	}
	if current < 59 {
		if err := s.applyEndpointRelayAuthorizationMigration(); err != nil {
			return err
		}
	}
	if current < 60 {
		if err := s.applyRelayDownstreamAuthorizationMigration(); err != nil {
			return err
		}
	}
	if current < 61 {
		if err := s.applyRelayUploadWindowMigration(); err != nil {
			return err
		}
	}
	if current < 62 {
	...
	if current < 64 {
		if err := s.applyDelayedHeartbeatGraceMigration(); err != nil {
			return err
		}
	}
	if current < 65 {
		if err := s.applyOrganizationBoundaryMigration(); err != nil {
			return err
		}
	}
	if current < 66 {
		if err := s.applyOrganizationScopePolicyMigration(); err != nil {
			return err
		}
	}
	return nil
