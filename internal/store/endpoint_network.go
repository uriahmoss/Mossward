...
func (s *SQLiteStore) RecordEndpointNetworkInventory(endpointID string, inventory model.EndpointNetworkInventory, receivedAt time.Time) error {
	...
	for index, connection := range inventory.Connections {
		if _, err := tx.Exec(`INSERT INTO endpoint_network_connections(endpoint_id,ordinal,protocol,local_address,local_port,remote_address,remote_port,process_id,process_name,direction,executable,remote_hostname,hostname_source,tls_server_name) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			endpointID, index, connection.Protocol, connection.LocalAddress, connection.LocalPort, connection.RemoteAddress, connection.RemotePort, connection.ProcessID, connection.ProcessName, connection.Direction, connection.Executable, connection.RemoteHostname, connection.HostnameSource, connection.TLSServerName); err != nil {
			return err
		}
	}
...
func (s *SQLiteStore) EndpointNetworkInventory(endpointID string) (model.EndpointNetworkInventory, error) {
	...
	rows, err := s.db.Query(`SELECT protocol,local_address,local_port,remote_address,remote_port,process_id,process_name,direction,executable,remote_hostname,hostname_source,tls_server_name FROM endpoint_network_connections WHERE endpoint_id=? ORDER BY ordinal`, endpointID)
	if err != nil {
		return inventory, err
	}
	defer rows.Close()
	for rows.Next() {
		...
		if err := rows.Scan(&connection.Protocol, &connection.LocalAddress, &connection.LocalPort, &connection.RemoteAddress, &connection.RemotePort, &connection.ProcessID, &connection.ProcessName, &connection.Direction, &connection.Executable, &connection.RemoteHostname, &connection.HostnameSource, &connection.TLSServerName); err != nil {
			return inventory, err
		}
