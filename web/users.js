...
async function loadScannerWorkers() {
  ...
  document.querySelector("#workers-list").innerHTML = workers.length ? workers.map((worker) => `<article class="session-row"><div><strong>${escapeHTML(worker.name)}</strong><span>${escapeHTML(worker.status)} · ${worker.allowed_cidrs.map(escapeHTML).join(", ")} · ${worker.allowed_ports.length} ports · assigned concurrency ${worker.max_concurrent} · available ${worker.available_concurrency} · ${worker.rate_limit_per_second ? `${worker.rate_limit_per_second} checks/sec` : "unlimited rate"} · ${escapeHTML(worker.software_version || "version unknown")} on ${escapeHTML(worker.operating_system || "unknown OS")}/${escapeHTML(worker.architecture || "unknown architecture")} · capabilities: ${(worker.capabilities || []).map(escapeHTML).join(", ") || "not reported"} · certificate expires ${new Date(worker.expires_at).toLocaleString()}${worker.last_seen_at ? ` · last seen ${new Date(worker.last_seen_at).toLocaleString()}` : " · never connected"}${worker.alerts?.length ? ` · ${worker.alerts.map((alert) => escapeHTML(alert.message)).join("; ")}` : ""}${worker.revocation_reason ? ` · ${escapeHTML(worker.revocation_reason)}` : ""}</span></div>${worker.status === "active" ? `<button class="worker-revoke compact-button" data-id="${worker.id}">Revoke</button>` : ""}</article>`).join("") : "No scanner workers enrolled.";
