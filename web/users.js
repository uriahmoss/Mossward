...
async function revokeEndpoint(id) {
  const reason = window.prompt("Reason for revoking this endpoint certificate:");
  if (!reason) { return; }
  const response = await secureFetch(`/api/admin/endpoints/${encodeURIComponent(id)}/revoke`, {method: "POST", headers: csrfHeaders, body: JSON.stringify({reason})});
...
async function loadScannerWorkers() {
  ...
  document.querySelector("#workers-list").innerHTML = workers.length ? workers.map((worker) => `<article class="session-row"><div><strong>${escapeHTML(worker.name)}</strong><span>fleet ${escapeHTML(worker.fleet_state || "unknown")} · ${escapeHTML(worker.status)} · dispatch ${worker.dispatch_enabled ? "enabled" : "paused"} · site ${escapeHTML(worker.site_id || "any")} · ${worker.allowed_cidrs.map(escapeHTML).join(", ")} · ${worker.allowed_ports.length} ports · assigned concurrency ${worker.max_concurrent} · available ${worker.available_concurrency} · ${worker.active_jobs || 0} active jobs reserving ${worker.reserved_concurrency || 0} slots · ${worker.rate_limit_per_second ? `${worker.rate_limit_per_second} checks/sec` : "unlimited rate"} · ${escapeHTML(worker.software_version || "version unknown")} on ${escapeHTML(worker.operating_system || "unknown OS")}/${escapeHTML(worker.architecture || "unknown architecture")} · capabilities: ${(worker.capabilities || []).map(escapeHTML).join(", ") || "not reported"} · certificate expires ${new Date(worker.expires_at).toLocaleString()}${worker.last_seen_at ? ` · last seen ${new Date(worker.last_seen_at).toLocaleString()}` : " · never connected"}${worker.alerts?.length ? ` · ${worker.alerts.map((alert) => escapeHTML(alert.message)).join("; ")}` : ""}${worker.revocation_reason ? ` · ${escapeHTML(worker.revocation_reason)}` : ""}</span></div>${worker.status === "active" ? `<button class="worker-dispatch compact-button" data-id="${worker.id}" data-enabled="${worker.dispatch_enabled}">${worker.dispatch_enabled ? "Pause jobs" : "Allow jobs"}</button><button class="worker-revoke compact-button" data-id="${worker.id}">Revoke</button>` : ""}</article>`).join("") : "No scanner workers enrolled.";
...
document.querySelector("#worker-enrollment-form").addEventListener("submit", async (event) => {
  ...
  const request = {name: document.querySelector("#worker-name").value,
    site_id: document.querySelector("#worker-site").value.trim(),
